...
manage w = whenX (not <$> isClient w) $
    withDisplay $ \d -> do
        ...
        let adjust (W.RationalRect x y wid h)
            ...
            f ws
                | shouldFloat = W.float w (adjust rr) . W.insertUp w . W.view i $ ws
                | otherwise = W.insertUp w ws
              where
                i = W.tag $ W.workspace $ W.current ws
...
windows f = do
    ...
    rects <- fmap concat $
        forM (zip allscreens summed_visible) $ \(w, vis) -> do
            ...
            (rs, ml') <-
                runLayout wsp{W.stack = tiled} viewrect
                    `catchX` runLayout wsp{W.stack = tiled, W.layout = Layout Full} viewrect
            updateLayout n ml'
...
rescreen :: X ()
rescreen =
    withDisplay getCleanedScreenInfo >>= \case
...
-}
focus :: Window -> X ()
focus w = local (\c -> c{mouseFocused = True}) $
    withWindowSet $ \s -> do
        let stag = W.tag . W.workspace
            curr = stag $ W.current s
        mnew <-
            maybe (return Nothing) (fmap (fmap stag) . uncurry pointScreen)
                =<< asks mousePosition
        root <- asks theRoot
        case () of
            _
                | W.member w s && W.peek s /= Just w -> windows (W.focusWindow w)
                | Just new <- mnew
                  , w == root && curr /= new ->
                    windows (W.view new)
                | otherwise -> return ()
...
initColor dpy c =
    C.handle (\(C.SomeException _) -> return Nothing) $
  ...
  where
    go = withDisplay $ \d -> do
        ...
        let sr_eq = (==) `on` fmap (screenRect . W.screenDetail)
            ...
            rr =
                if managed || point_sc `sr_eq` Just sc
...
mouseDragCursor cursorGlyph f done = do
    ...
    case drag of
        ...
        Nothing -> do
            ...
            io $ do
                cursor <- maybe (pure none) (createFontCursor d) cursorGlyph
                grabPointer
                    d
                    root
                    False
...
mouseMoveWindow w = whenX (isClient w) $
    withDisplay $ \d -> do
        wa <- io $ getWindowAttributes d w
...
mouseResizeWindow w = whenX (isClient w) $
    withDisplay $ \d -> do
        wa <- io $ getWindowAttributes d w
        sh <- io $ getWMNormalHints d w
        ...
        mouseDragCursor
            ...
            ( \ex ey -> do
                io $
                    resizeWindow d w
                        `uncurry` applySizeHintsContents
                            sh
                            ( ex - fromIntegral (wa_x wa)
